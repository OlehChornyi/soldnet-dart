...
class SearchBottomSheet extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final selectedParam =
        ref.watch(storeSearchProvider.select((state) => state.selectedParam));
    ...
    return AppBottomSheet(
        ...
        body: Column(
          ...
          spacing: 8,
          children: [
            for (var p in SearchParam.values) ...{
              SearchBottomSheetItem(
                name: p.label,
                isSeltcted: selectedParam == p,
                onTap: () {
                  searchNotifier.setSelectedParam(p);
                  context.pop();
                },
              )
            },
